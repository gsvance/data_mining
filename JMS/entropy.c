...
int main(int argc, char *argv[])
{
	...
	for (h=1; h < argc; ++h)
	{
		...
		fprintf(ofp, "ID, X_Pos, Y_Pos, Z_Pos, Temp, U, U_dot, rho, V_x, V_y, V_z \n");
		for (i = 0; i < nobj; ++i)
		{
			fread(&part,sizeof(particle),1,fp);
			fprintf(ofp,"%d, %g, %g, %g, %g, %g, %g, %g, %g, %g, %g \n", 
				part.ident, part.x, part.y, part.z, part.temp, part.u, part.udot, part.rho, part.vx, part.vy, part.vz);
		}
