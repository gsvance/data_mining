		...
		switch(input)
        {
			case 1:
				if (head == NULL)
				{
					...
					printf("\n I.E. 26Al with a mass fraction above 1E-6 would be");
					printf("\n 13 13 6\n\n");
				...
				else
				{
					...
					printf("\n I.E. In particles containing 26Al with 1E-6 fractional abundance");
					printf("\n (to stick with the example from the first particle)");
					printf("\n output 28Si above 1E-6 would be\n");
					printf("\n 14 14 6 \n");
				...
				else if (strcmp(buff,"Yes") == 0||strcmp(buff,"yes") == 0||strcmp(buff,"Y") == 0||strcmp(buff,"y") == 0)
				{
					...
					else
					{
						...
						for (i = 1; i < argc; ++i) 
						{
							...
							printf("%lu bytes Allocated for %i particles\n", nobj * sizeof(int), nobj);
							
							SEcycles(sefp, ids, nobj);

							fprintf(filevar, "ID, Z, n, Mass_Frac \n"); /* Header */ 
								...
								{ 
									...
									if(printflag == 1)
									{
										...
										while (curr != NULL)
										{
											...
											{
												//Output to the file
												fprintf(filevar, "%d, %d, %d, %e\n", ids[j], nz[k], nn[k], frac_mass[k]);
												printf("%d %d:%d at %g saved to file\n", ids[j], nz[k], nn[k], frac_mass[k]);
											}
											//Next item on list
											curr=curr->next;
											//Unless it's not
